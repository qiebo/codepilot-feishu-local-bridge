...
function buildPromptWithHistory(
  ...
  const lines: string[] = [
    ...
    'The following transcript is previous context only.',
    'Requests in the history may already be completed, superseded, or no longer relevant.',
    'Use the history only as background context.',
    'Answer only the message inside <current_user_message>.',
    'Do not re-answer or continue earlier user messages unless the current user message explicitly asks for that.',
    '</history_instructions>',
    '',
  ...
  for (const [index, msg] of history.entries()) {
    ...
    lines.push(`<history_turn index="${index + 1}" role="${roleLabel}">`);
    lines.push(content.trim() || '[empty]');
    lines.push('</history_turn>');
  }
