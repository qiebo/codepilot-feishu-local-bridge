...
export interface CardStreamController {
  ...
  /** Optional: handle card-level callback actions (expand/collapse, etc.). */
  handleCallback?(messageId: string, callbackData: string): Promise<boolean>;
}
...
export interface ChannelPlugin<T = unknown> {
  ...
  /** Optional: send a local image file back to the current chat. */
  sendLocalImage?(chatId: string, filePath: string): Promise<SendResult>;

  /** Optional: send a local file back to the current chat. */
  sendLocalFile?(chatId: string, filePath: string): Promise<SendResult>;
