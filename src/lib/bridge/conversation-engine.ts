...
const HUMAN_GATE_SYSTEM_PROMPT = [
  ...
  'For sites with strong anti-automation checks such as X/Twitter, Google, Apple, GitHub, or similar login pages, do not default to Playwright or other automation browsers for the login step unless the user explicitly asks for that approach.',
  'If the user says they can see or control the virtual desktop or browser, prefer opening the target page in a visible normal browser session and then stop the turn so the user can complete the login manually.',
  'Do not create helper scripts whose only purpose is to keep a browser open indefinitely.',
  'Never end a login helper script with an infinite wait such as await new Promise(() => {}), a forever loop, or any other blocking construct that keeps the bridge occupied.',
  'If you open a browser for manual login, launch it in a non-blocking way when possible, send a screenshot or checkpoint, explain what is waiting, and then stop the turn.',
...
const TOOL_DISCOVERY_SYSTEM_PROMPT = [
  ...
  'For browser login tasks, prefer an existing browser session, existing browser profile, or project-native login tool over ad-hoc Playwright automation.',
  'If the target site is likely to block automation, prefer letting the user complete the login in a visible normal browser instead of generating a custom automation script.',
...
function buildRuntimeModelSystemPrompt(effectiveModel?: string, providerId?: string): string {
  const lines = [
    'For this bridge session, the configured runtime model information below is authoritative.',
    `Configured bridge session model: ${effectiveModel || 'unknown'}`,
    `Configured provider id: ${providerId || 'unknown'}`,
    'If the user asks which model is currently in use, answer from the configured bridge session model above instead of guessing.',
    'Ignore plugin, hook, memory, or debug text such as "Agent\'s current model" when identifying the active runtime model.',
    'In particular, claude-subconscious / Letta plugin diagnostics may mention a separate agent model and must not be treated as the current Claude Code bridge model.',
  ];

  return lines.join('\n');
}

function loadMcpServers(workingDirectory?: string): Record<string, MCPServerConfig> | undefined {
  try {
    const merged = resolveEffectiveMcpServers(workingDirectory);
...
export type ProgressUpdate =
  ...
  | { kind: 'tool_progress'; toolName: string; toolUseId?: string; elapsedSeconds: number }
...
export async function processMessage(
  ...
  try {
    ...
    const mcpServers = loadMcpServers(effectiveWorkingDirectory);
    ...
    if (binding.channelType === 'feishu') {
      ...
      systemPromptParts.push(buildRuntimeModelSystemPrompt(effectiveModel, resolvedProvider?.id || session?.provider_id || undefined));
    ...
    const effectiveSystemPrompt = systemPromptParts.filter(Boolean).join('\n\n').trim() || undefined;
...
async function consumeStream(
  ...
  try {
        ...
        switch (event.type) {
          ...
          case 'tool_use': {
            ...
            try {
              ...
              if (onProgressUpdate && toolData?.name) {
                try {
                  onProgressUpdate({ kind: 'tool_started', toolName: String(toolData.name) });
                } catch { /* non-critical */ }
              }
              if (onToolEvent && toolData?.id && toolData?.name) {
                try {
                  onToolEvent({
                    type: 'tool_use',
          ...
          case 'tool_result': {
            try {
              ...
              const newBlock: Extract<MessageContentBlock, { type: 'tool_result' }> = {
                type: 'tool_result',
              ...
              if (onToolEvent && resultData?.tool_use_id) {
                try {
                  onToolEvent({
                    type: 'tool_result',
                    tool_use_id: String(resultData.tool_use_id),
                    is_error: Boolean(resultData.is_error),
                    content,
                  });
                } catch { /* non-critical */ }
              }
              ...
              if (toolOutputData?._progress && toolOutputData.tool_name) {
                try {
                  onProgressUpdate({
                    ...
                    toolUseId: typeof toolOutputData.tool_use_id === 'string' ? toolOutputData.tool_use_id : undefined,
    ...
    if (cleanedBlocks.length > 0) {
      const hasToolBlocks = cleanedBlocks.some((b) => b.type === 'tool_use' || b.type === 'tool_result');
