...
export abstract class BaseChannelAdapter {
  ...
  /**
   * Send a local image file to the channel.
   * Default implementation reports that the adapter does not support it.
   */
  async sendLocalImage(_chatId: string, _filePath: string): Promise<SendResult> {
    return { ok: false, error: 'Local image sending is not supported by this adapter' };
  }

  /**
   * Send a local file to the channel.
   * Default implementation reports that the adapter does not support it.
   */
  async sendLocalFile(_chatId: string, _filePath: string): Promise<SendResult> {
