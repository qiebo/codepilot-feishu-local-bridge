...
function summarizeArtifactFailures(failures: string[]): string {
  if (failures.length === 1) {
    return `Attachment send failed: ${failures[0]}`;
  }
  return `Some attachments could not be sent:\n- ${failures.join('\n- ')}`;
}
...
function shouldPersistSdkResumeForBinding(): boolean {
  return false;
}
...
async function handleMessage(
  ...
  try {
    ...
    if (binding.id && shouldPersistSdkResumeForBinding()) {
      try {
