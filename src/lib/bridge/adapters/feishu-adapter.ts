...
function inferFeishuFileType(filePath: string): 'opus' | 'mp4' | 'pdf' | 'doc' | 'xls' | 'ppt' | 'stream' {
...
export class FeishuAdapter extends BaseChannelAdapter {
  ...
  override async sendLocalImage(chatId: string, filePath: string): Promise<SendResult> {
    const imageResult = await this.trySendLocalImage(chatId, filePath);
    if (imageResult.ok) return imageResult;

    console.warn(
      '[feishu-adapter] Image send failed, falling back to file:',
      imageResult.error || filePath,
    );
    return this.trySendLocalFile(chatId, filePath);
  }

  override async sendLocalFile(chatId: string, filePath: string): Promise<SendResult> {
    return this.trySendLocalFile(chatId, filePath);
  }
  ...
  private async trySendLocalImage(chatId: string, filePath: string): Promise<SendResult> {
    if (!this.restClient) {
      return { ok: false, error: 'Feishu client not initialized' };
    }
    ...
    try {
      ...
      if (stat.size <= 0) {
        return { ok: false, error: `Image is empty: ${path.basename(filePath)}` };
      }
      if (stat.size > MAX_OUTBOUND_IMAGE_SIZE) {
        return { ok: false, error: `Image exceeds 10 MB limit: ${path.basename(filePath)}` };
      }
      ...
      if (!imageKey) {
        return { ok: false, error: `Image upload did not return image_key: ${path.basename(filePath)}` };
      }
      ...
      if (res?.data?.message_id) {
        try {
          insertAuditLog({
            ...
            summary: `[image] ${path.basename(filePath)}`,
          });
        } catch { /* best effort */ }
        return { ok: true, messageId: res.data.message_id };
      }

      return { ok: false, error: res?.msg || `Failed to send image: ${path.basename(filePath)}` };
    } catch (err) {
  ...
  private async trySendLocalFile(chatId: string, filePath: string): Promise<SendResult> {
    ...
    try {
      ...
      if (stat.size > MAX_OUTBOUND_FILE_SIZE) {
        return { ok: false, error: `File exceeds 30 MB limit: ${path.basename(filePath)}` };
      }
      ...
      if (!fileKey) {
        return { ok: false, error: `File upload did not return file_key: ${path.basename(filePath)}` };
      }
      ...
      return { ok: false, error: res?.msg || `Failed to send file: ${path.basename(filePath)}` };
    } catch (err) {
      return { ok: false, error: err instanceof Error ? err.message : `Failed to send file: ${path.basename(filePath)}` };
    }
  }
